...
void window(int i) {
  ...
  setText((168 - w) / 2, 40 , "Fluid Level " + String(i) , false , 0, 20 , COLOR_BLACK , 2 , FreeSans12pt7b);
...
void windowValues(int i) {
  ...
  if (int(hight[i]) != int(hightTemp[i]) || counter != counterTemp || i != iTemp) {
    ...
    int gray = 220 - (storage[i] * 220)/100;
    tft.fillRectangle(0, 0, 178, gray, COLOR_GRAY); //(x1,y1,x2,y2,color)
    tft.fillRectangle(0, gray, 178, 220, COLOR_BLUE); //(x1,y1,x2,y2,color)
    window(i+1);
    counter++;
    int w;
    //tft.fillRectangle(40, 90, 140, 115, COLOR_BLACK); //(x1,y1,x2,y2,color)
    w = getTextLength(hightStr[i]);
    setText((168 - w) / 2, 105 , hightStr[i] , false, 0, 20 , COLOR_BLACK);
    //tft.fillRectangle(30, 135, 150, 155, COLOR_BLACK); //(x1,y1,x2,y2,color)
    w = getTextLength(amountstr[i]);
    setText((168 - w) / 2, 150 , amountstr[i] , false, 0, 20 , COLOR_BLACK);
    //tft.fillRectangle(40, 180, 140, 200, COLOR_BLACK); //(x1,y1,x2,y2,color)
    w = getTextLength(storagestr[i]);
    setText((168 - w) / 2, 195 , storagestr[i] , false, 0, 20 , COLOR_BLACK);
  }
