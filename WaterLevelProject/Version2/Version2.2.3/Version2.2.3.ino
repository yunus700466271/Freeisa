...
void percentageBar(int x , int y, int Percentage , int, int, int); //Show Percentage Bar
...
void updateValues(int); //Read new values from sensors
...
void window(int i) {
  ...
  int w;
  w = getTextLength("Tank " + String(i) + " level", FreeSans12pt7b);
  setText((168 - w) / 2, 40 ,"Tank " + String(i) + " level", false , 0, 20 , COLOR_LIGHTBLUE , 2 , FreeSans12pt7b);
}



void percentageBar(int x , int y, int Percentage = 0, int widthOfDegree = 60, int numberOfDegrees = 55, int sizeOfDegree = 1) {

  int i = 0, green, red = numberOfDegrees;

  if (Percentage >= 0 && Percentage <= 100) {
    green = (numberOfDegrees * Percentage) / 100;
  }
  else if (Percentage < 0) {
    green = 0;
  } else {
    green = numberOfDegrees;
  }

  for (; i < green ; i++) {
    tft.fillRectangle(x , y, x + widthOfDegree, y + sizeOfDegree, COLOR_BLUE); //set background
    y -= sizeOfDegree * 2;
  }
  for (; i < red ; i++) {
    tft.fillRectangle(x , y, x + widthOfDegree, y + sizeOfDegree, COLOR_GRAY); //set background
    y -= sizeOfDegree * 2;
  }
}
...
void windowValues(int i) {
  ...
  i -= 1;
  percentageBar(100, 200, storage[i]);
  hightStr[i] = String(hight[i] / 10) + " Cm";
  amountstr[i] = String(amount[i]) + " L";
  ...
  if (int(hight[i]) != int(hightTemp[i]) || counter != counterTemp || i != iTemp) {
    ...
    tft.fillRectangle(5, 90, 90, 115, COLOR_BLACK); //(x1,y1,x2,y2,color)
    setText(10, 105 , hightStr[i] , false, 0, 20 , COLOR_WHITE);
    tft.fillRectangle(5, 135, 90, 155, COLOR_BLACK); //(x1,y1,x2,y2,color)
    setText(10, 150 , amountstr[i] , false, 0, 20 , COLOR_WHITE);
    tft.fillRectangle(5, 180, 90, 200, COLOR_BLACK); //(x1,y1,x2,y2,color)
    setText(10, 195 , storagestr[i] , false, 0, 20 , COLOR_WHITE);
